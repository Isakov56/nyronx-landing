...
function FlagUZ({ className = "w-5 h-3.5 rounded-[3px] object-cover shadow-sm" }) {
  return (
    <svg className={className} viewBox="0 0 640 480" fill="none">
      ...
      <rect y="152" width="640" height="16" fill="#CE1126" />
      <rect y="312" width="640" height="16" fill="#CE1126" />
      <circle cx="70" cy="80" r="30" fill="#FFFFFF" />
      <circle cx="82" cy="80" r="25" fill="#0099B5" />
      <g fill="#FFFFFF">
        <circle cx="140" cy="50" r="6" /><circle cx="170" cy="50" r="6" /><circle cx="200" cy="50" r="6" /><circle cx="230" cy="50" r="6" /><circle cx="260" cy="50" r="6" />
        <circle cx="170" cy="80" r="6" /><circle cx="200" cy="80" r="6" /><circle cx="230" cy="80" r="6" /><circle cx="260" cy="80" r="6" />
        <circle cx="200" cy="110" r="6" /><circle cx="230" cy="110" r="6" /><circle cx="260" cy="110" r="6" />
      </g>
    </svg>
  )
}

function FlagRU({ className = "w-5 h-3.5 rounded-[3px] object-cover shadow-sm" }) {
  return (
    <svg className={className} viewBox="0 0 640 480" fill="none">
...
function FlagEN({ className = "w-5 h-3.5 rounded-[3px] object-cover shadow-sm" }) {
  return (
    <svg className={className} viewBox="0 0 640 480" fill="none">
    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        >
          ...
          {/* Right side: Flag Language Dropdown + Demo CTA */}
          <div className="ml-auto flex items-center gap-2.5">
            {/* 3-Language Dropdown Selector with Flag */}
            <div className="relative" ref={langDropdownRef}>
              <button
                type="button"
                onClick={() => setLangDropdownOpen((o) => !o)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gray-100/90 hover:bg-gray-200/80 border border-black/[0.06] text-xs font-bold text-brand-ink transition-all cursor-pointer shadow-xs"
                aria-haspopup="true"
              ...
              >
                ...
                <ChevronDown
                  className={`w-3 h-3 text-gray-500 transition-transform duration-200 ${langDropdownOpen ? 'rotate-180' : ''}`}
                />
              </button>

              {/* Dropdown Menu */}
              {langDropdownOpen && (
                <div className="absolute right-0 top-full mt-2 w-40 rounded-2xl bg-white p-1.5 shadow-xl border border-black/10 z-[70] animate-in fade-in zoom-in-95 duration-150">
                  {LANGUAGES.map((lang) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[10px] text-gray-400 font-mono uppercase">{lang.short}</span>
                      </button>
                    )
                  })}
                </div>
            ...
            >
              {navNavbar.contact || (language === 'uz' ? 'Demo olish' : language === 'ru' ? 'Получить демо' : 'Get Demo')}
            </button>
      ...
      {mobileOpen && (
        ...
        >
          <div className="px-6 py-6 flex flex-col gap-2 font-semibold text-brand-ink">
            ...
            <div className="flex items-center gap-2 p-1 rounded-2xl bg-gray-100/90 border border-black/5 mb-4">
              {LANGUAGES.map((lang) => {
                const isActive = language === lang.code
                return (
                  <button
